...
let ProductInformation = Meteor.ProductInformation;

let ProductInformationHeaders = Meteor.ProductInformationHeaders;
import { collProductInformations } from '../../lib/collections.js';
...
Meteor.validatorFunctions = {
    ...
    onClickFindInValidData: function(sheetName) {
        ...
        getHeaders(sheetName, function(err, sheetName) {
            ...
            findInValidData(sheetName, currentRule, ProductInformation, function(err, productData) {
                if (!err) {
                    //check column avaialble or not
                    let columnName = ProductInformation[currentRule].validationGroup;
...
function updateDataMongo(guid, columnName, newValue) {
    ...
    let dataResult = collProductInformations.update({
        _id: guid
...
function findInvalidDataFromMongo(sheetName, currentRuleIdx, arrRules, callback) {
    ...
    console.log(query);
    let result = collProductInformations.find(query).fetch();
    if (result.length <= 0) {
